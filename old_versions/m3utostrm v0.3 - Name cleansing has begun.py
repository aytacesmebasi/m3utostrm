    ...
    while i < len(lines):
        ...
        if extinf_line.startswith('#EXTINF:'):
            if i + 1 < len(lines):
                ...
                # Film veya dizi adını temizle ve TMDb'de ara
                is_tv = bool(re.search(r'\s*S\d{2}\s*E\d{2}', media_name))
                cleaned_media_name = clean_name(media_name, is_tv)
                print(f"'{media_name}' temizlenmiş haliyle '{cleaned_media_name}' olarak aranıyor.")
                tmdb_data = search_tmdb(cleaned_media_name, is_tv)
                
                # TMDb'den bilgi varsa NFO dosyasını oluştur
                if tmdb_data:
                    if is_tv:
                ...
                print(f"Uyarı: {extinf_line} için URL eksik.")
                i += 1  # Son satırdayız, sadece ilerle
        else:
